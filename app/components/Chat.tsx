...
const GLOBAL_CONVERSATION_ID =
  process.env.NEXT_PUBLIC_GLOBAL_CONVERSATION_ID ??
...
export default function Chat({ user }: { user: ChatUserShape }) {
  ...
  const [unreadCountByConversationId, setUnreadCountByConversationId] =
  ...
  const [dmSortOrder, setDmSortOrder] = useState<
    "newest" | "oldest" | "az" | "za"
  >("newest");
  ...
  const privateConversations = conversations
    ...
    .sort((a, b) => {
      ...
      const aName =
        a.users.find((u) => u.id !== user.id)?.email?.split("@")[0] || "";
      const bName =
        b.users.find((u) => u.id !== user.id)?.email?.split("@")[0] || "";
  ...
  const activeOtherUser = activeConversation?.users.find(
    (u) => u.id !== user.id,
  );
  ...
  return (
    <>
      ...
      <div className="flex h-screen w-full bg-transparent text-white overflow-hidden relative">
        {/* Left Sidebar */}
        <div
          className={`w-full md:w-[300px] flex-shrink-0 border-r border-white/5 flex flex-col bg-[#0a0a1a] md:bg-transparent z-20 absolute md:relative h-full transition-transform duration-300 ${conversationId ? "-translate-x-full md:translate-x-0" : "translate-x-0"}`}
        >
          <div className="p-5 border-b border-white/5">
            <div className="flex items-center justify-between mb-4">
              ...
              >
                <FontAwesomeIcon
                  icon={faPlus}
                  className="text-indigo-400 w-3.5 h-3.5"
                />
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/30">
            <div>
              <h3 className="px-2 text-xs font-semibold text-gray-500 mb-2 tracking-wider">
                ROOMS
              </h3>
              <Conversations
              ...
              <div className="flex justify-between items-center px-2 mb-2 relative">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {dmSortOrder === "za" && "Z-A"}
                    <FontAwesomeIcon
                      icon={isDmSortOpen ? faChevronUp : faChevronDown}
        ...
        {/* Middle Chat Area */}
        <div
          className={`flex-1 flex flex-col min-w-0 relative bg-white/[0.01] ${!conversationId ? "hidden md:flex" : "flex"}`}
        >
          {conversationId ? (
            <>
              {/* Header */}
              <div className="h-[72px] flex items-center justify-between px-4 sm:px-6 border-b border-white/5 bg-white/[0.01] z-10 flex-shrink-0">
                <div className="flex items-center gap-2 sm:gap-3.5">
                  ...
                  <div className="relative">
                    <div
                      className={`flex justify-center items-center w-11 h-11 rounded-full text-[16px] font-bold shadow-sm ${isGlobalActive ? "bg-indigo-500/15 text-indigo-300 border border-indigo-500/30" : "bg-neutral-800 text-gray-200 border border-white/10"}`}
                    >
                      {activeInitials}
                    </div>
                  ...
                  <div>
                    <h2 className="text-[16px] font-bold text-gray-100 leading-tight">
                      {activeLabel}
                    </h2>
                    <p className={`text-xs font-medium ${activeSublabelClass}`}>
                      {activeSublabel}
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="mb-[2px] pr-1">
                    ...
                    >
                      ...
                      <FontAwesomeIcon
                        icon={faPaperPlane}
                        className={`w-[14px] h-[14px] transition-transform ${canSendMessage ? "translate-x-0.5" : ""}`}
                      />
                    </button>
                  </div>
                </div>
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center h-full">
              ...
              <p className="text-gray-500 text-sm max-w-sm">
                Select a conversation from the left sidebar or start a new one
                to begin chatting.
              </p>
            </div>
          )}
        ...
        {conversationId && (
          <div
            className={`w-full sm:w-[320px] flex-shrink-0 border-l border-white/5 flex flex-col absolute right-0 top-0 bottom-0 h-full z-40 bg-[rgba(10,10,30,0.95)] md:bg-[#0a0a1a] xl:bg-transparent xl:relative xl:transform-none transition-transform duration-300 ${showRightSidebar ? "translate-x-0" : "translate-x-full xl:translate-x-0 xl:hidden"}`}
          >
            <div className="absolute top-4 right-4 xl:hidden">
              <button
                onClick={() => setShowRightSidebar(false)}
                className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-gray-300 hover:text-white"
              >
            ...
            <div className="flex flex-col items-center justify-center p-8 border-b border-transparent">
              <div
                className={`flex justify-center items-center w-[100px] h-[100px] rounded-full text-4xl font-bold mb-5 shadow-lg ${isGlobalActive ? "bg-indigo-500/10 text-indigo-300 border border-indigo-500/20" : "bg-neutral-800 text-gray-200 border border-white/10"}`}
              >
                {activeInitials}
              </div>
              ...
              <div className="flex items-center gap-3 mt-6 w-full justify-center">
                <button
                  ...
                  className="w-11 h-11 rounded-2xl bg-[rgba(10,10,30,0.6)] flex items-center justify-center text-gray-400 hover:text-indigo-400 hover:bg-white/5 transition shadow-sm"
                ...
                <button
                  ...
                  className={`w-11 h-11 rounded-2xl bg-[rgba(10,10,30,0.6)] flex items-center justify-center text-gray-400 hover:bg-white/5 transition shadow-sm ${isGlobalActive ? "opacity-50 cursor-not-allowed" : "hover:text-red-400"}`}
                ...
                <button
                  ...
                  className="w-11 h-11 rounded-2xl bg-[rgba(10,10,30,0.6)] flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/5 transition shadow-sm"
                  title="Info"
                >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/30 border-t border-white/5">
              <div>
                <div className="flex items-center justify-between cursor-pointer group mb-4">
                  <h3 className="text-[14px] font-semibold text-gray-300 group-hover:text-white transition tracking-wide">
                    Shared Media
                  </h3>
                  <FontAwesomeIcon
                    icon={faChevronDown}
                    className="text-gray-500 w-3.5 h-3.5 group-hover:text-gray-300 transition"
                  />
                </div>
                ...
                {allMediaAttachments.length > 0 ? (
                  <>
                    <div className="grid grid-cols-3 gap-2">
                      {allMediaAttachments
                        .slice(0, showAllMedia ? undefined : 9)
                        .map((att, i) => (
                          <div
                            key={i}
                            className="aspect-square bg-neutral-900 rounded-xl overflow-hidden border border-transparent shadow-sm relative group cursor-pointer"
                            onClick={() =>
                              setMediaViewer({
                                type: att.mimetype?.startsWith("video/")
                                  ? "video"
                                  : "image",
                                url: att.public_url,
