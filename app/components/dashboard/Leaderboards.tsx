...
export default function Leaderboards() {
  ...
  const createLeaderboard = async () => {
    ...
    const createPromise = fetch("/api/leaderboards", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: sanitizedLeaderboardName }),
    }).then(async (res) => {
      if (!res.ok) {
    ...
    toast.promise(createPromise, {
      ...
      error: {
        render({ data }) {
          return (
            (data as Error)?.message || "Failed to create. Please try again."
          );
        },
      },
    });
  ...
  const joinLeaderboard = async () => {
    ...
    toast.promise(joinPromise, {
      ...
      error: {
        render({ data }) {
          return (
            (data as Error)?.message || "Failed to join. Please try again."
          );
        },
      },
    });
                      ...
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(
                            `${window.location.origin}/join?id=${createdCode}`,
                          );
                          setCopied("url");
                          setTimeout(() => setCopied(null), 2000);
                        }}
              ...
              <>
                ...
                <div className="flex items-start justify-between p-6 pb-4 border-b border-gray-200 relative z-10">
                  <div className="flex items-center gap-4">
                    ...
                    >
                      <FontAwesomeIcon
                        icon={activeModal === "create" ? faPlus : faUsers}
                        className="w-5 h-5"
                      />
                    </div>
                    <div className="flex flex-col">
